...
# setting the DEST variable with  cloumn number for Dest env in report_final.csv

DEST=$(cat   report_final.csv |grep 'module-name'| tr ',' "\n"| nl | grep "$dest_node"| awk '{print $1}')

# setting the Source variable with  cloumn number for Dest env in report_final.csv
SOURCE=$(cat   report_final.csv |grep 'module-name'| tr ',' "\n"|nl | grep "$src_node"| awk '{print $1}')


# report_final1.csv file is being created like  module1,1.1.10
cat report_final.csv | grep -v "module-name"|cut -d, -f1,$SOURCE >  report_final1.csv
# report_final1.csv file is being created like  1.1.10
cat report_final.csv | grep -v "module-name"|cut -d, -f$DEST >  report_final2.csv
...
root_final > report_final.csv
if [[ $? -eq 0 ]]
then
        echo "Final Report file stored at report_final.csv"
else
...
root_master_compare > Report_filter.csv
